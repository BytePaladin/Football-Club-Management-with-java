        ...
        try (BufferedReader reader = new BufferedReader(new FileReader("player.txt"))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (parts.length >= 7) {
                    String name = parts[0];
                    String pass = parts[1];
                    int jerseyNumber = Integer.parseInt(parts[2]);
                    int id = Integer.parseInt(parts[3]);
                    String startDateStr = parts[4];
                    String endDateStr = parts[5];
                    double contractMoney = Double.parseDouble(parts[6]);
                    String healthStatus = parts[7];

                    Player player = new Player(new Person(name, pass), jerseyNumber,id, startDateStr, endDateStr, contractMoney, healthStatus);
                    players.add(player);
                }
            }
            reader.close();
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("player.txt"))) {
            for (Player player : players) {
                writer.write(player.getPerson().getName() + "," + player.getPerson().getPassword() + ","
                        + player.getJerseyNumber() + "," +player.getId()+","+ player.getStartDate() + ","
                        + player.getEndDate() + "," + player.getContractMoney() + "," + player.getHealthStatus()
                        + "\n");
